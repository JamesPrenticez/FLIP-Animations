...
function Item(props) {
    ...
    return (<ItemContainer style={baseStyles} ref={setNodeRef} tabIndex={0} layoutId={id} animate={transform
            ? {
                x: transform.x,
                y: transform.y,
...
export default function FramerMotion() {
    const [items, setItems] = useState(() => createRange(16, (index) => (index + 1).toString()));
    ...
    function handleDragEnd({ over }) {
        setItems((items) => arrayMove(items, items.indexOf(activeId), items.indexOf(over.id)));
        setActiveId(null);
    }
    return (<>
      <button onClick={() => setItems(arrayMove(["1", "2", "3"], 0, 2))} style={{ height: "100px", width: "100px" }}>
        hi
      </button>
      <Wrapper>
